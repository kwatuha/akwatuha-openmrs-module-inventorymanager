...
import org.openmrs.module.amrsreport.rule.EvaluableRule;
  ...
  * Author jmwogi
  */
public class LostToFollowUpRule  extends EvaluableRule {
 
 	private static final Log log = LogFactory.getLog(LostToFollowUpRule.class);
	...
	public Result evaluate(LogicContext context, Integer patientId, Map<String, Object> parameters) throws LogicException {
	 try {
		...
		EncounterType encTpInit = Context.getEncounterService().getEncounterType(EvaluableNameConstants.ENCOUNTER_TYPE_ADULT_INITIAL);
		EncounterType encTpRet = Context.getEncounterService().getEncounterType(EvaluableNameConstants.ENCOUNTER_TYPE_ADULT_RETURN);
		// DEAD
		for (Iterator<Encounter> it = e.iterator(); it.hasNext();) {
		    ...
		    for (Iterator<Obs> obs = o.iterator();obs.hasNext();) {
				...
				if((encTpInit == encounter.getEncounterType()) || (encounter.getEncounterType() == encTpRet)){
					if(ob.getConcept().getConceptId() == Context.getConceptService().getConcept(EvaluableNameConstants.RETURN_VISIT_DATE).getConceptId()){
