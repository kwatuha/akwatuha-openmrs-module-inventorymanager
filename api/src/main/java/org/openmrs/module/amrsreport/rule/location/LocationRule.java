...
public class LocationRule extends EvaluableRule {

	private static final Log log = LogFactory.getLog(LocationRule.class);
	...
	protected Result evaluate(LogicContext context, Integer patientId,Map<String, Object> parameters) {
		...
		Concept TRANSFER_CARE_TO_OTHER_CENTER=Context.getConceptService().getConcept(EvaluableNameConstants.TRANSFER_CARE_TO_OTHER_CENTER);
		Concept WITHIN_AMPATH_CLINICS=Context.getConceptService().getConcept(EvaluableNameConstants.WITHIN_AMPATH_CLINICS);
		Concept REASON_FOR_MISSED_VISIT=Context.getConceptService().getConcept(EvaluableNameConstants.REASON_FOR_MISSED_VISIT);
		Concept TRANSFER_IN_WITHIN_AMPATH_CLINICS=Context.getConceptService().getConcept(EvaluableNameConstants.AMPATH_CLINIC_TRANSFER);
		Concept TRANSFER_CARE_TO_OTHER_CENTER_DETAILED =Context.getConceptService().getConcept(EvaluableNameConstants.TRANSFER_CARE_TO_OTHER_CENTER_DETAILED);
		Concept FREETEXT_GENERAL =Context.getConceptService().getConcept(EvaluableNameConstants.FREETEXT_GENERAL);
		Concept NON_AMPATH  =Context.getConceptService().getConcept(EvaluableNameConstants.NON_AMPATH);
		Concept REASON_EXITED_CARE  =Context.getConceptService().getConcept(EvaluableNameConstants.REASON_EXITED_CARE);
		Concept TRANSFER_OUT_FROM_THE_PROGRAM  =Context.getConceptService().getConcept(EvaluableNameConstants.PATIENT_TRANSFERRED_OUT);
		Concept OUTCOME_AT_END_OF_TUBERCULOSIS_TREATMENT  =Context.getConceptService().getConcept(EvaluableNameConstants.OUTCOME_AT_END_OF_TUBERCULOSIS_TREATMENT);
