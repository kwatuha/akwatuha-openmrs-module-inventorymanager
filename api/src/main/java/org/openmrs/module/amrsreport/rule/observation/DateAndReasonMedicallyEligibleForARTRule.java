...
import org.openmrs.module.amrsreport.rule.EvaluableNameConstants.AgeGroup;
 ...
 */
public class DateAndReasonMedicallyEligibleForARTRule extends EvaluableRule {

	private static final Log log = LogFactory.getLog(DateAndReasonMedicallyEligibleForARTRule.class);
	...
	private class PatientFlags {
		...
		public Boolean consume(Obs o) {
			...
			if (q.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_ADULT))) {
				if (answer.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_1_ADULT))) {
					adultWHOStage = 1;
				} else if (answer.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_2_ADULT))) {
					adultWHOStage = 2;
				} else if (answer.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_3_ADULT))) {
					adultWHOStage = 3;
				} else if (answer.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_4_ADULT))) {
					adultWHOStage = 4;
				}
				return true;
			}
			if (q.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_PEDS))) {
				if (answer.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_1_PEDS))) {
					pedsWHOStage = 1;
				} else if (answer.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_2_PEDS))) {
					pedsWHOStage = 2;
				} else if (answer.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_3_PEDS))) {
					pedsWHOStage = 3;
				} else if (answer.equals(getCachedConcept(EvaluableNameConstants.WHO_STAGE_4_PEDS))) {
					pedsWHOStage = 4;
				}
				return true;
			}
			if (q.equals(getCachedConcept(EvaluableNameConstants.HIV_DNA_PCR))) {
				if (answer.equals(getCachedConcept(EvaluableNameConstants.POSITIVE))) {
					HIVDNAPCRPositive = true;
					return true;
				}
			}
			if (q.equals(getCachedConcept(EvaluableNameConstants.CD4_BY_FACS))) {
				cd4ByFacs = value;
				return true;
			}
			if (q.equals(getCachedConcept(EvaluableNameConstants.CD4_PERCENT))) {
	...
	private List<Concept> getQuestionConcepts() {
		if (cachedQuestions == null) {
			...
			cachedQuestions.add(getCachedConcept(EvaluableNameConstants.CD4_BY_FACS));
			cachedQuestions.add(getCachedConcept(EvaluableNameConstants.CD4_PERCENT));
			cachedQuestions.add(getCachedConcept(EvaluableNameConstants.HIV_DNA_PCR));
			cachedQuestions.add(getCachedConcept(EvaluableNameConstants.WHO_STAGE_PEDS));
			cachedQuestions.add(getCachedConcept(EvaluableNameConstants.WHO_STAGE_ADULT));
		}
